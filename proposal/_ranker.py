...
class ProposedRanker(Ranker):
    def __init__(self, lr: float, warmup_steps: int, alpha: float=0.5, sparsity_tgt: float=3, topk=1.0, cache_dir: Union[str, Path, None] = "./cache/colbert/") -> None:
        ...
        self.save_hyperparameters({"lr": lr, "warmup_steps": warmup_steps, "alpha": alpha, "sparsity_tgt": sparsity_tgt})
    ...
    @torch.no_grad()
    def _fw_colbert_single_or_load_from_cache(self, input_ids: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
    ...
    def _forward_colbert_representation_or_load_from_cache(self, input: dict[str, torch.LongTensor]) -> torch.Tensor:
        out = torch.stack([self._fw_colbert_single_or_load_from_cache(*pair) for pair in zip(input["input_ids"], input["attention_mask"])])
        return out
